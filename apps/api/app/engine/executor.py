...
def _merge_context(context: dict[str, Any], output: dict[str, Any]) -> Optional[dict[str, Any]]:
...
async def resume_workflow_run(run_id: UUID, *, approved: bool, note: str = "") -> None:
    """Continue a paused approval node, or fail the run on rejection."""
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(WorkflowRun)
            .options(selectinload(WorkflowRun.workflow), selectinload(WorkflowRun.node_runs))
            .where(WorkflowRun.id == run_id)
        )
        run = result.scalar_one_or_none()
        if not run or run.status != RunStatus.paused.value:
            return
        ...
        waiting = next(
            (
                ...
                if nr.status == NodeRunStatus.waiting_approval.value
            ),
            None,
        )
        if not waiting or not pause:
            run.status = RunStatus.failed.value
            run.error_message = "Paused run is missing approval checkpoint"
            run.completed_at = _utcnow()
            await db.commit()
        ...
        if not approved:
            waiting.status = NodeRunStatus.failed.value
            waiting.error_message = note or "Approval rejected"
            waiting.completed_at = _utcnow()
            waiting.output_data = {
                **(waiting.output_data or {}),
                "approved": False,
                "note": note,
                "paused": False,
            }
            run.status = RunStatus.failed.value
            run.error_message = note or f"Approval rejected at node {waiting.node_id}"
            run.completed_at = _utcnow()
            if run.started_at:
                run.duration_ms = int((run.completed_at - run.started_at).total_seconds() * 1000)
            run.output_payload = None
            run.logs = list(run.logs or []) + [
                {"level": "error", "message": f"Approval rejected at node {waiting.node_id}"}
            ]
            await db.commit()
            await _emit(
                run.id,
                "node.status",
                {"node_id": waiting.node_id, "status": waiting.status, "error": waiting.error_message},
            )
            await _emit(run.id, "run.status", {"status": run.status, "error": run.error_message})
            return
        ...
        snapshot = (waiting.output_data or {}).get("context_snapshot")
        context: dict[str, Any] = dict(snapshot if isinstance(snapshot, dict) else (run.input_payload or {}))
        context["approval"] = {"approved": True, "note": note, "node_id": waiting.node_id}

        waiting.status = NodeRunStatus.completed.value
        waiting.output_data = {
            **(waiting.output_data or {}),
            "approved": True,
            "note": note,
        ...
        waiting.error_message = None
        waiting.completed_at = _utcnow()

        executed: set[str] = set(pause.get("executed") or [])
        skipped: set[str] = set(pause.get("skipped") or [])
        indegree = {str(k): int(v) for k, v in (pause.get("indegree") or {}).items()}
        # Ensure every node has an indegree entry after JSON round-trip.
        for nid in nodes:
            indegree.setdefault(nid, 0)
        final_output = pause.get("final_output")
        ready: list[str] = []

        executed.add(waiting.node_id)
        ...
        run.logs = list(run.logs or []) + [
            {"level": "info", "message": f"Approval granted at node {waiting.node_id}"}
        ]
        await db.commit()
        await _emit(
            run.id,
            "node.status",
            {"node_id": waiting.node_id, "status": waiting.status, "output": waiting.output_data},
        )
        await _emit(run.id, "run.status", {"status": run.status})
...
async def _execute_loop(
    ...
    *,
    ...
    while ready:
        ...
        maybe_final = _merge_context(context, output)
        ...
        if output.get("paused"):
            snapshot = {k: v for k, v in context.items() if k != "context_snapshot"}
            node_run.output_data = {**output, "context_snapshot": snapshot}
...
async def enqueue_decision(run_id: UUID, approved: bool, note: str = "") -> None:
    try:
        await resume_workflow_run(run_id, approved=approved, note=note)
    except Exception:  # noqa: BLE001
        async with AsyncSessionLocal() as db:
            result = await db.execute(select(WorkflowRun).where(WorkflowRun.id == run_id))
            run = result.scalar_one_or_none()
