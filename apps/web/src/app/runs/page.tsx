    ...
    >
      ...
      {!loading && !error ? (
        <div className="mb-4 grid gap-3 sm:grid-cols-4">
          ...
          <SummaryCard label="Tokens" value={String(summary.totalTokens)} />
          <SummaryCard label="Est. cost" value={`$${summary.totalCost.toFixed(4)}`} />
      ...
      {!loading && !error && runs.length > 0 ? (
        <div className="panel overflow-hidden">
          ...
          <p className="border-t border-slate-100 px-4 py-2 text-xs text-slate-500">
            Avg duration (filtered set): {summary.avgDuration || 0} ms
          </p>
        </div>
      ) : null}
    </DashboardShell>
